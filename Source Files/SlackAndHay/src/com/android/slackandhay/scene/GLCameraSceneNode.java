...
public class GLCameraSceneNode extends GLNullSceneNode {
	...
	public void switchToPerspView(final GL10 gl) {
		...
		assert gl11 != null;
		gl.glViewport(-4, -4, (int) _width + 4, -(int) _height - 4); // widened
																		// viewport
		gl.glMatrixMode(GL11.GL_PROJECTION);
		gl.glLoadIdentity();
		GLU.gluPerspective(gl, angle * zoom, (float) _width / (float) _height,
				1, zFar - zNear);
		update(gl);
	}
	...
	public void switchToOrthView(final GL10 gl) {
		...
		gl.glViewport(-4, -4, (int) _width + 4, -(int) _height - 4); // widened
																		// viewport
		gl.glOrthof(-(float) (_width / _height), (float) (_width / _height),
				-1, 1, -1, 1);
	...
	private void update(GL10 gl) {
		GLU.gluLookAt(gl, eye[0], eye[1], eye[2], focus[0], focus[1], focus[2],
				up[0], up[1], up[2]);
