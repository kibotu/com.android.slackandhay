...
public abstract class GOComponentGraphics extends GOComponent {
	...
	public void update(final int dt) {
		switch (parent.getStateMananger().getActiveState().getStateType()) {
		case IDLE:
			break;
		case WALKING:
			break;
	...
	/**
	 * sets current texture id based on progress percentage
	 * and variable array length does nothing, when percentages haven't changed
	 * 
	 * @param ids
	 */
	...
	public void setImage(final int id) {
		...
		setTextureVboOffset(id*32); // 32 = 4 x * 4 y texture coords * 4 bytes each quad
	}
