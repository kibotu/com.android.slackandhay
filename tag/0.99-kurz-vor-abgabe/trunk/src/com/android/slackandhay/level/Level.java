...
public class Level {
	 ...
	 * 
	 * @return
	 * 		a pointF for spawning or null, if everything is occupied.
	 */
	...
	/**
	 * Generates a <code>Grid</code> and a Vector of <code>SpawnPoints</code> out of a given XML File resource id.
	 * 
	...
	public GridWorld load(final int levelRessourceID) {
		...
		try {
			final Document doc = saxBuilder.build(new DataInputStream(new BufferedInputStream(activity.getResources().openRawResource(levelRessourceID))));
			List<Element> list = doc.getRootElement().getChildren("world");
			Element eWorld = list.get(0);
			gridWorld = new GridWorld(eWorld.getAttribute("width").getIntValue(), eWorld.getAttribute("height").getIntValue(),new PointF(0, 0),eWorld.getAttribute("scale").getFloatValue());
	...
	private void addSpawnPoint(Element e) throws DataConversionException {
		for(SpawnPoint.GameObjectType type: SpawnPoint.GameObjectType.values()) {
			if(e.getAttributeValue("type").equals(type.name().toLowerCase())) {
				spawnPoints.add(new SpawnPoint((int)e.getAttribute("x").getFloatValue(),(int) e.getAttribute("y").getFloatValue(), e.getAttribute("width").getFloatValue(),e.getAttribute("height").getFloatValue(), type));
